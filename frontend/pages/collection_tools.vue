  ...
  <div>
    ...
    <Column field="url" header="URL">
        <template #body="slotProps">
            <a :href="getURL(slotProps.data.url)" about="_blank">{{ slotProps.data.url }}</a>
        </template>
    </Column>
    <Column>
        <template #body="slotProps">
            <Button @click="editTool(slotProps.data)" label="Edit" icon="pi pi-pencil" rounded outlined />
        </template>
    </Column>
...
const UploadDialog = defineAsyncComponent(() => import('../components/create_tool.vue'))
const EditDialog = defineAsyncComponent(() => import('../components/edit_tool.vue'))

const get_tools = () => {
//   datasets.value = fake_data
//   return
  axios.get('/api/collectionTools')
    .then(response => {
...
const deleteData = () => {
    ...
    confirm.require({
        ...
        accept: () => {
            ...
            to_delete.forEach(d => {
                axios.delete(`/api/collectionTools/${encodeURIComponent(d.name)}`)
                    .then(() => {
                        toast.add({
                            severity: 'success',
